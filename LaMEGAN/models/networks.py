...
from torchvision.transforms import InterpolationMode as T
...
# To define which feature layer should be extracted, see the noteForVGG.txt
...
# Feature extractor for Cycle-Dehaze
class VGGFeatureExtractor_CycleDehaze(nn.Module):
    # Extract the 2nd and 5th maxpooling layers of the VGG16
    def __init__(self, use_bn=False, use_input_norm=True,
                 device=torch.device('cuda')):
        super(VGGFeatureExtractor_CycleDehaze, self).__init__()
        self.use_input_norm = use_input_norm
        if use_bn:
        ...
        second_layer = 9
        fifth_layer = 30
        self.second_features = nn.Sequential(*list(model.features.children())[:(second_layer + 1)]).to(device)
        self.fifth_features = nn.Sequential(*list(model.features.children())[:(fifth_layer + 1)]).to(device)
        
        # No need to BP to variable
        for k, v in self.second_features.named_parameters():
            v.requires_grad = False
        for k, v in self.fifth_features.named_parameters():
            v.requires_grad = False
    ...
    def forward(self, x):
        ...
        if self.use_input_norm:
            x = (x - self.mean) / self.std
        output_second = self.second_features(x)
        output_fifth = self.fifth_features(x)
        return output_second, output_fifth
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netG == 'rrdb_2blocks':
        net = RRDBNetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, n_blocks=2)
    elif netG == 'rrdb_3blocks':
        net = RRDBNetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, n_blocks=3)
    elif netG == 'rrdb_6blocks':
        net = RRDBNetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, n_blocks=6)
...
def define_F(feature_layer=9, gpu_ids=[]):
    net = VGGFeatureExtractor(feature_layer=feature_layer)
    if len(gpu_ids) > 0:
        assert(torch.cuda.is_available())
        net.to(gpu_ids[0])
        net = torch.nn.DataParallel(net, gpu_ids)  # multi-GPUs
...
def define_F_CycleDehaze(gpu_ids=[]):
    net = VGGFeatureExtractor_CycleDehaze()
    if len(gpu_ids) > 0:
        assert(torch.cuda.is_available())
        net.to(gpu_ids[0])
        net = torch.nn.DataParallel(net, gpu_ids)  # multi-GPUs
...
def define_featNet(which_model_netFeat='resnet34', gpu_ids=[]):
    netFeat = None
    ...
    if which_model_netFeat == 'resnet34':
        ...
        raise NotImplementedError('Feature model name [%s] is not recognized' %
                                  which_model_netFeat)
    if use_gpu:
        netFeat.to(gpu_ids[0])
        netFeat = torch.nn.DataParallel(netFeat, gpu_ids) # multi-GPUs
    return netFeat
...
#### Designing RRDB networks
#### Code is modified from ESRGAN's repository (https://github.com/xinntao/BasicSR)
...
### RRDBGenerator is here ###
class ResidualDenseBlock(nn.Module):
    """Residual Dense Block.

    Used in RRDB block in ESRGAN with ReLU activation layer.

    Args:
        num_feat (int): Channel number of intermediate features.
        num_grow_ch (int): Channels for each growth.
        norm_layer: Default InstanceNorm2D. After every Conv2d
    """

    def __init__(self, num_feat=64, num_grow_ch=32, norm_layer=nn.InstanceNorm2d):
        super(ResidualDenseBlock, self).__init__()
        self.conv1 = nn.Conv2d(num_feat, num_grow_ch, 3, 1, 1)
        self.norm1 = norm_layer(num_grow_ch)

        self.conv2 = nn.Conv2d(num_feat + num_grow_ch, num_grow_ch, 3, 1, 1)
        self.norm2 = norm_layer(num_grow_ch)

        self.conv3 = nn.Conv2d(num_feat + 2 * num_grow_ch, num_grow_ch, 3, 1,
                               1)
        self.norm3 = norm_layer(num_grow_ch)

        self.conv4 = nn.Conv2d(num_feat + 3 * num_grow_ch, num_grow_ch, 3, 1,
                               1)
        self.norm4 = norm_layer(num_grow_ch)

        self.conv5 = nn.Conv2d(num_feat + 4 * num_grow_ch, num_feat, 3, 1, 1)
        self.norm5 = norm_layer(num_feat)
    ...
    def forward(self, x):
        ...
        x2 = self.relu(self.norm2(self.conv2(torch.cat((x, x1), 1))))
        x3 = self.relu(self.norm3(self.conv3(torch.cat((x, x1, x2), 1))))
        x4 = self.relu(self.norm4(self.conv4(torch.cat((x, x1, x2, x3), 1))))
        x5 = self.norm5(self.conv5(torch.cat((x, x1, x2, x3, x4), 1)))
...
class RRDB(nn.Module):
    ...
    def __init__(self, num_feat, num_grow_ch=32, norm_layer=nn.InstanceNorm2d):
        super(RRDB, self).__init__()
        self.rdb1 = ResidualDenseBlock(num_feat, num_grow_ch, norm_layer)
        self.rdb2 = ResidualDenseBlock(num_feat, num_grow_ch, norm_layer)
        self.rdb3 = ResidualDenseBlock(num_feat, num_grow_ch, norm_layer)
...
class RRDBNetGenerator(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.

    We adapt Torch code and idea from Justin Johnson's neural style transfer project(https://github.com/jcjohnson/fast-neural-style)
    """

    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.InstanceNorm2d, n_blocks=3):
        """Construct a Resnet-based generator
        ...
        Parameters:
            ...
            output_nc (int)     -- the number of channels in output images
            ngf (int)           -- the number of filters in the last conv layer
            n_blocks (int)      -- the number of RRDB blocks
            padding_type (str)  -- the name of padding layer in conv layers: reflect | replicate | zero
        """
        assert(n_blocks >= 0)
        super(RRDBNetGenerator, self).__init__()

        conv_first = []
        conv_body = []
        conv_last = []

        conv_first += [nn.ReflectionPad2d(3),
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            conv_first += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=True),
                norm_layer(ngf * mult * 2),
                nn.ReLU(True)]

        self.conv_first = nn.Sequential(*conv_first)

        mult = 2 ** n_downsampling
        # add RRDBV2 blocks
        for i in range(n_blocks):
            conv_body += [RRDB(ngf * mult, num_grow_ch=32, norm_layer=norm_layer)]

        self.conv_body = nn.Sequential(*conv_body)

        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            conv_last += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                         kernel_size=3, stride=2,
                                         padding=1, output_padding=1,
                                         bias=True),
                      norm_layer(int(ngf * mult / 2)),
                      nn.ReLU(True)]
        conv_last += [nn.ReflectionPad2d(3)]
        conv_last += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        conv_last += [nn.Tanh()]
