...
class MIINetModel(BaseModel):
	"""This class implements the MIINet model
	MIINet paper: https://arxiv.org/abs/2011.14132
	"""
	@staticmethod
	def modify_commandline_options(parser, is_train=True):
		...
		if is_train:
			...
			parser.add_argument('--lambda_percep', type=float, default=0.5, help='perceptual loss coefficient')
	...
	def __init__(self, opt):
		...
		self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'percep_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'percep_B']
	...
	def forward(self):
		...
		# perceptual loss
		if(self.isTrain):
			self.fea_real_A = self.netF(self.real_A).detach()
			self.fea_fake_B = self.netF(self.fake_B)

			self.fea_real_B = self.netF(self.real_B).detach()
			self.fea_fake_A = self.netF(self.fake_A)
	...
	def backward_G(self):
		...
		if(self.isTrain):
			...
			self.loss_percep_A = 0.0
			self.loss_percep_B = 0.0
